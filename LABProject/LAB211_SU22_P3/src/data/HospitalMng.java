...
public class HospitalMng {
    ...
    public void addPatient() {
        if (doctorList.isEmpty()) {
            System.out.println("Please add at least 1 doctor!");
            return;
        }
        System.out.println("----------ADD PATIENT PROGRAM----------");
        String doctorID;
        boolean OK;
        doctorID = MyUtil.readNonBlankStr("Doctor ID");
        OK = doctorList.contains(new Doctor(doctorID));
        if (OK == false) {
            System.out.println("Not found!");
        } else {
            String patientID = MyUtil.readNonBlankStr("Patient ID");
            int pos = patientList.indexOf(new Patient(patientID));
            if (pos >= 0) {
                System.out.println("ID duplicated!");
            } else {
                String patientName = MyUtil.readNonBlankStr("Patient name");
                int age = MyUtil.readInt("Age", 0);
                String address = MyUtil.readNonBlankStr("Address");
                String examID = MyUtil.readNonBlankStr("Examination ID");
                String result = null;
                Date date = null;
                Patient patient = new Patient(patientID, patientName, age, address);
                patientList.add(patient);
                System.out.println(patient);
                patientList.writeToFile("patient.dat");
                Examination exam = new Examination(examID, doctorID, patientID, result, date);
                examList.add(exam);
                System.out.println(exam);
                examList.writeToFile("examination.dat");
                System.out.println("A patient ID: " + patientID + " was added successfully.");
            }
        }
    }

    public void RecordMedical() {
        String doctorID;
        boolean OK;
        doctorID = MyUtil.readNonBlankStr("Doctor ID");
        OK = doctorList.contains(new Doctor(doctorID));
        if (OK == false) {
            ...
            String patientID = MyUtil.readNonBlankStr("Patient ID");
            OK = patientList.contains(new Patient(patientID));
            if (OK == false) {
                System.out.println("Not found!");
            } else {
                String examID = MyUtil.readNonBlankStr("Examination ID");
                OK = examList.contains(new Examination(examID));
                if (OK == false) {
                    System.out.println("Not found!");
                } else {
                    String result = MyUtil.readNonBlankStr("Result");
                    Date date = new Date(System.currentTimeMillis());
                    Examination exam = new Examination(examID, doctorID, patientID, result, date);
                    examList.add(exam);
                    System.out.println(exam);
                    examList.writeToFile("examination.dat");
                    System.out.println("A examination ID: " + examID + " was record successfully.");
                }
            }
        }
    }
